...
if len(sys.argv) > 1:
   if sys.argv[1] == "record":
       # TODO check that dotnet process is already started.
       url = "http://localhost:61417"
       dotnet_process = subprocess.Popen(["dotnet", "run", "CompatibilitySuite/obj/Debug/netstandard2.1/Program.exe", "record"])
   elif sys.argv[1] == "playback":
       url = "http://localhost:61417"
       dotnet_process = subprocess.Popen(["dotnet", "run", "CompatibilitySuite/obj/Debug/netstandard2.1/Program.exe", "playback"])
...
# TODO warn that dotnet process was not started.
